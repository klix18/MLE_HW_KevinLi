...
from step6_sqlite import load_into_sqlite
import glob
...
def setup_rag(skip_scrape=False, skip_extract=False, skip_chunk=False, skip_embed=False, skip_faiss=False, skip_sqlite=False):
    # Step 1: Scrape
    if skip_scrape:
    ...
    # Step 6: SQLite (metadata + chunks + FTS)
    if not skip_sqlite:
        load_into_sqlite(docs_json=DOCS_JSON)
    else:
        print("Skipping SQLite ingest")
